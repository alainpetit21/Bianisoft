...
public class FactoryRegistry extends AudioDeviceFactory
{
	...
	final protected Hashtable<Class, AudioDeviceFactory> factories = new Hashtable<Class, AudioDeviceFactory>();
	...
	public AudioDevice createAudioDevice() throws JavaLayerException
	{
		...
		AudioDeviceFactory[] audDevFactories = getFactoriesPriority();
		
		if (audDevFactories==null)
			throw new JavaLayerException(this+": no factories registered");
		
		JavaLayerException lastEx = null;
		for (int i=0; (device==null) && (i<audDevFactories.length); i++)
		{
			try
			{
				device = audDevFactories[i].createAudioDevice();
			}
