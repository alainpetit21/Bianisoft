...
public class WavInputStream extends InputStream {
	...
	public WavInputStream( Replay replay ) {
		this.replay = replay;
		mixBuf = new int[ replay.getMixBufferLength() ];
		outBuf = new byte[ mixBuf.length * 4 ];
		int duration = replay.calculateSongDuration();
		int samplingRate = replay.getSamplingRate();
	...
	private void getAudio() {
		int mEnd = replay.getAudio( mixBuf ) * 2;
	...
	public static void main( String[] args ) throws java.io.IOException {
		if( args.length == 2 ) {
			...
			buf = new byte[ replay.getMixBufferLength() * 4 ];
			int remain = replay.calculateSongDuration() * 4 + header.length;
			while( remain > 0 ) {
